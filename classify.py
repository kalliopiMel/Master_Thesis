...
def Candidates_vs_Reference(text_list, measure):
    ...
    similarities = []
    # in this list we count how many times a candidate text was the most similar one
    candidate_rank = []
    ...
    for i in reference_rank:  # every text in the text list is a reference text.
        ...
        # and for every reference text i want to check the proximity with all the rest
        for j in range(text_list_length):
            ...
            if j != i:   # but i obviously have to leave out the reference text, since candidate 1 cannot also be the reference text
                # and every candidate 1 has to be compared with all the rest
                for k in range(j + 1, text_list_length):
                    ...
                    # pairwise_sim = pairwise_sim + i, " \t \t ", j, " \t \t ", sim1, "\t \t ", k, "\t \t", sim2, "\n"  # we also write everything down on a string or file
                        ...
                        if sim1 >= sim2:  # if sim1>=sim2 then the cand1 is closer to the reference than cand2, thus we give it one point, otherwise the point goes to cand2
                            # what we count here is how many times each text was the most proximate compared to anothter
                            candidate_rank[j] = candidate_rank[j] + 1
                        else:
                            candidate_rank[k] = candidate_rank[k] + 1

        for j in range(text_list_length):
            if j != i:  # again we don't need the reference text
                # creating the dictionary, where key is the ref text. Every key has a relative list, containing the other texts and how many times where found more similar than other
                reference_rank[i].append([j, candidate_rank[j]])
                # this creates a 2D array of this form [[ref, cand*, sim*],[ref,cand*,sim*],....] where cand* is the most similar of the two candidates and the relative similarity number sim*
                similarities.append([i, j, candidate_rank[j]])
    ...
    return reference_rank, similarities, pairwise_sim
